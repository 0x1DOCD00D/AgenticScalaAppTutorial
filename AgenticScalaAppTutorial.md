...
Everything in this tutorial rests on one fact about language models that is easy to state and easy to forget: a model produces text, and nothing else. It cannot touch a disk, start a process, open a network connection, or delete a table. Left entirely alone, an instruction like create build.sbt produces a beautifully formatted description of build.sbt.

A tool is the bridge between that text and the world, a named operation with a typed parameter schema that the harness, meaning Claude Code itself, offers to the model at the start of a session. Write is a tool whose parameters are a file path and content. Bash is a tool whose parameter is a command string. The model emits a structured request, called a tool call, naming the tool and supplying arguments, in a machine-readable block. The harness reads that block, performs the real operation with the real permissions of your operating system user, and appends the result, file contents, command output, an error, back into the model's context as text. The model reads the result and continues.

So every action in this tutorial is really three events in a loop. The model proposes, which is text. The harness executes, which is code. The model observes the result, which is text again. When [Phase 0](#phase-0-the-seed-agent) says the session will propose one Write tool call, it means exactly this: you will see a request to write one file, the harness will carry it out, and the transcript records both halves. An agent is nothing more mysterious than this loop running until the model decides it is done, and the whole reason it can be governed is that the middle step belongs to the harness rather than to the model.

That middle step is where the entire enforcement story of this project lives. The permission lists in settings.json are consulted by the harness before it executes a call, which is why a denied terraform apply stays denied no matter how the prompt argues. The PreToolUse guard hook runs on the harness side between the proposal and the execution, which is why it can stop a DROP TABLE that the model fully intended to run. The tools line in an agent's frontmatter is the offer list itself: a tool absent from that list is not discouraged or forbidden, it simply does not exist in that agent's world. The code-reviewer does not resist the temptation to edit files; there is no Edit in its context to call, which is the difference between a rule and a fence, and it is the entire content of Mechanism 3.

The observation tools are Read, Grep, and Glob, which open files, search contents, and match file patterns; every agent holds these, because an agent that cannot look is an agent that guesses. The mutation tools are Write, which creates or replaces a file, and Edit, which changes part of an existing one; who holds which is a policy decision, and the distinction is fine-grained enough that the dependency-updater holds Edit but not Write, so it can bump a version on an existing line but cannot create structure. Then there is Bash, which runs an arbitrary shell command and is the general-purpose escape hatch: compiles, tests, git, docker, the AWS CLI. Beyond the built-ins, WebSearch and WebFetch reach knowledge newer than the model's training data, and the mcp__server__tool names are operations served by separate server programs, described in [Appendix E](#appendix-e-the-mcp-servers-and-what-they-are-for).

Bash deserves one honest caveat, because it is the coarse spot in the fence. A shell can do almost anything, including writing files through redirection, so in principle an agent with Bash but without Write is not fully prevented from creating files. The fence is deliberately coarse there, and the design compensates in two layers: the guard hook inspects every proposed command for dangerous patterns, and the instruction layer assigns Bash a purpose per agent, evidence gathering for the reviewer, builds and tests for the implementer. This is why the tutorial never claims the fence alone is sufficient, and why Mechanism 5 exists beneath it. When you audit an agent's tools line, audit Bash by asking what the hooks would catch, not by assuming the fence catches everything.

The mental model to carry through the rest of the tutorial is that the model is a planner filling out order forms, the tools are the catalogue it may order from, the harness is the warehouse that fills the orders, and the hooks are the inspector at the loading dock. The tools line decides the catalogue per agent, permissions and hooks decide what the warehouse will actually ship, and the transcript keeps every order form, which is why nothing an agent does here is invisible. Choosing an agent's tools is therefore not configuration bookkeeping; it is deciding what kind of hands that agent has, and it deserves the same review attention as any law in its file.

---

### Mechanisms as built-in cause-and-effect pathways that operate the same way every time

The word *mechanism* means a fixed piece of causal machinery in the runtime: a built-in cause-and-effect pathway that operates the same way every time, whether or not anyone intends it, remembers it, or agrees with it. A mechanism is a gear, not a signpost or an empty declaration like All Lives Matter! The contrast class is convention or advice: "one phase per session" is a convention you follow; "the Stop hook exits 2 if tests did not run" is a mechanism that fires whether you follow anything or not. We list five mechanisms because they are the complete causal inventory of the system with every behavior you will ever observe in the tutorial traces back to exactly one of them. Routing by description is the machinery that selects which agent runs; the fresh context is the machinery that determines what the selected agent knows; the tool fence is the machinery that bounds what it can do; the instruction hierarchy inside the file is the machinery that shapes how it works; hooks and permissions are the machinery that constrains every act regardless of everything above. The phrase "and only five" hands you a debugging ontology. When something surprising happens, the diagnostic question is always "which of the five produced this?". One refinement is that the five are not equally deterministic. Fences, hooks, and permissions are mechanisms in the strictest sense, code with guaranteed outcomes. Routing and instruction-following run through the model, so they are statistical mechanisms: defined pathways with known operating characteristics but probabilistic output, which is exactly why the tutorial builds tests around them (the routing corpus) and backstops them with the deterministic three. Calling all five "mechanisms" is still right, because in every case the behavior flows through a nameable, inspectable pathway you can engineer.
...
The opening is a mission statement. The words a real request contains ("alarm", "5xx", "smoke test failed", "degraded") appear only at the end. A request typed at 3 a.m. reads "the 5xx alarm is firing"; the description whose opening contains "alarms fire... 5xx rates spike" wins instantly, while the mission-statement version competes on a fuzzy notion of reliability that the deploy-engineer's description also radiates. Front-loaded and effective command is below.
...
The sixth hard rule: audit the set, then test it empirically, and fix failures in the description rather than in your prompts. 
- The collision audit is to write ten realistic requests, assign each to an agent by hand, then check that no request's wording plausibly matches two descriptions; where it does, sharpen the trigger clauses until exactly one wins. 
- The orphan audit is to check that every lifecycle stage's natural vocabulary appears in some description; a stage that matches nothing will be handled inline by the orchestrator, which means by nobody with laws. 
- Then probe live by phrasing a request without naming any agent and see who picks it up. If the wrong agent answers, the temptation is to just name the right agent in future prompts. Resist it; explicit naming is a workaround that lives in your head, while a corrected description is a fix that works for every future session, every teammate, and every scheduled run.

**Mechanism 2: the fresh context**. When an agent is invoked, the runtime assembles a new context containing three things only: the agent file's body (which becomes its system prompt), the project memory (CLAUDE.md plus files it imports), and your task prompt. The agent does not see your conversation, other agents' work, or its own previous runs. As a consequence, everything an agent must always know has to be in its file or in CLAUDE.md, and anything one agent must tell another has to travel through the prompts, e.g., you paste report text forward. For dependencies between agents read [Appendix F](#appendix-f-dependencies-between-agents-the-blocked-on-protocol).
...
What enforcement actually means is that when the runtime spawns a subagent, it reads the frontmatter `tools:` list and offers the model only those tools. A tool absent from the list does not exist in that context: it is not shown to the model, the model cannot propose a call to it, and a hallucinated call to an absent tool fails at the runtime with an unknown-tool error rather than executing. This is enforcement by absence, which is categorically different from the other mechanisms: permissions and hooks filter invocations of tools the agent has; the fence removes whole capabilities so there is nothing to filter. An instruction says "do not edit"; the fence arranges that there is no edit to do. That difference is why the enforcement ladder puts the fence at the top: it is the only rung where violation is not merely blocked but inexpressible.

The trap in the default - an omitted `tools:` field does not mean no tools; it means all tools, everything the parent session has, including every connected MCP tool. This inverts the safe intuition when omitting a grant denies it, and it turns a careless deletion of one line into the widest possible agent. Two consequences for the writer are to never omit the field, in any agent and to treat any diff that touches a `tools:` line as an authority change requiring the matrix to change first. The factory-engineer's own laws encode both.

The unit of the fence is a whole tool: Read, Grep, Glob are the perception bundle nearly every agent gets; Edit and Write are separate tools, and the difference is usable, for example, the dependency-updater has Edit but not Write, since it can modify existing build files but cannot create new ones, which fences "bump a version" apart from "add a file" at the capability level; WebSearch and WebFetch are granted only where the role is research, because research tools invite unrequested research; and MCP tools are named individually (`mcp__postgres__run_query`), so granting capability and granting access are the same act. What the fence cannot express is anything inside a tool's argument space - for example, it cannot say "Write, but only under src/test" or "Bash, but only sbt". Those constraints drop down the ladder, to permission rules and hooks (deterministic, argument-aware) or to instruction plus detection (the test-engineer's src/test-only rule is prose, and the reviewer checks every diff's paths). The writer's discipline is to know, for every constraint in the authority matrix, which rung it lives on, and to write it there rather than wishing the fence could hold it.

A fence is derived mechanically, from the matrix's MAY column: list the tool calls the agent's procedure actually makes, grant exactly those, then stop. The reviewer's procedure reads diffs and runs sbt as evidence: Read, Grep, Glob, Bash, and pointedly no Edit or Write. The migrator writes migration files and inspects the live schema: add Write and the one MCP read tool. If you cannot name the procedure step that needs a tool, the tool goes; if the agent later fails for lack of it, that failure is loud and cheap (an unknown-tool error and a report), while the opposite error, a grant without a need, is silent standing risk. Over-fencing has a real failure mode where an agent whose procedure requires a verification step it lacks the tool for will either thrash or skip verification, so always check the fence against the agent's own procedure section, tool by tool.

Of course, Bash or other shell is a superset, since it can create files (`echo > file`), fetch URLs (`curl`), and generally reach most of what the other tools do. So "no Edit, no Write, but Bash granted" does not make writing physically impossible; it removes the legitimate write path, which changes the violation from invisible to anomalous. The residue is covered by the layers below where permission rules and the guard hook see every Bash command string, `git diff` makes any file the reviewer somehow wrote immediately visible as an anomaly in its own review, and the instruction "you never edit files" stands. The writer must hold this honestly, since the fence's guarantee is absolute only for tool-shaped access; where Bash is present, the guarantee is fence plus floor plus evidence, not fence alone. If a role must be absolutely read-only, drop Bash too, and accept the trade even though the reviewer would lose the ability to run the build as evidence. This project keeps Bash on the reviewer and accepts the layered guarantee, and that is a design decision you should make consciously, not by default.

Fences are configuration that is verified empirically by asking a new agent to do the thing it must not (e.g., "edit this file" to the reviewer) and expect a refusal; ask it to list its available tools and compare against the frontmatter; after any MCP server change, re-probe agents that hold MCP names, because a renamed server silently renames its tools and the agent loses the capability without any error at load time. And because a fence line is authority, its lifecycle is constitutional: proposed by the factory-engineer with a matrix justification, flagged by the reviewer's ownership axis, in force only after your ratification and a session restart.
...
The phase walkthroughs below point at specific laws by number, so you can open the agent file and see the exact sentence that caused the behavior. The five sections are the required functions, where every agent body must serve five functions such as identity, constraints, sequence, boundaries, contract, and may add more sections only when the role's shape demands them. In fact the project's own ten agents already demonstrate this, since several of them have headings beyond the canonical five, and each extra heading is there for a nameable reason. Walking through them gives you the complete list of justified additions.
...
*Scope*. The dependency-updater has a "Scope" section, for example, "you move VERSIONS only; structural build changes belong to build-engineer". It exists when two agents share one file. When artifact ownership splits inside a single file (the version ledger versus the structure of build.sbt), the boundary cannot be expressed as file paths, so it needs its own section drawing the line in terms of intent. If your ownership map never splits within a file, you never need this section.

*Cautions, or tripwires*. The dependency-updater's "Cautions specific to this stack" section (doobie RC imports move; http4s milestones are banned; upickle majors trip JsonCodecSuite) is the one section designed to grow over time, since every incident adds a line. It is warranted for two role types: maintenance agents that run unattended (no human present to supply context), and any role whose domain has recurring, nameable traps. For other agents the same content lives inside the iron laws; a separate section is justified when the list is open-ended rather than fixed.

*Mission with enumerable categories*. The test-engineer opens with "Mission" instead of a bare role line to find what the implementation missed, then four enumerable categories (boundaries, illegal transitions, malformed input, concurrency). This is the role line expanded, and the expansion is warranted specifically for adversarial or search roles, where the job is enumeration and adjectives ("write good tests") produce nothing. If the role's output is a search result, give it the search taxonomy as a section.
...
The economic rule is that the length budget is fixed even though the section list is not. An agent body stays around sixty lines; adding a Preconditions or Cautions section should usually be paid for by tightening something else, because six sections competing for salience weaken each other exactly the way ten iron laws do. So we always keep five functions, entry gates for the irreversible, scope lines for shared files, tripwire lists for the trap-prone and unattended, taxonomies for the adversarial, and nothing else without a failure that demands it.

**Mechanism 5: hooks and permissions, defined in `.claude/settings.json`.** These run outside the model and cannot be talked out of anything. Three hooks matter more than anything alse. The PostToolUse hook runs after every Edit or Write and formats Scala files (it always exits 0, so it never blocks). The PreToolUse hook inspects every shell command before it runs and exits 2 to block dangerous patterns such as DROP TABLE or terraform destroy; the text it prints to stderr is shown to the agent, which is why a blocked agent changes course instead of retrying. The Stop hook runs when an agent tries to finish; if Scala sources changed but the test suite has not run since (checked through a marker file that the build's `check` alias touches), it exits 2 and the agent is sent back to run `sbt check`. The permission lists in the same file pre-approve routine commands (sbt, git add/commit, terraform plan) and deny catastrophic ones (terraform apply, force-push) for everyone, agents and orchestrator alike. More information about hooks is available in [Appendix F](#appendix-f-dependencies-between-agents-the-blocked-on-protocol).

---

### What a law actually is

A law is the opposite kind of object from a tool, and the contrast is the fastest way to understand it. A tool is enforced by the harness: the code-reviewer cannot edit because no Edit exists in its world, and no amount of persuasion changes that. A law is not enforced by anything. It is a sentence in a markdown file, and nothing in the harness parses it, checks it, or executes it. When build-engineer law 1 says every version is a named `val`, no machinery anywhere knows what a named `val` is.

Recall from the tools explanation that every action is a loop: the model proposes, the harness executes, the model observes. Tools, permissions, and hooks all live in the execution step. Laws live in the proposal step because an agent's file body becomes its system prompt (Mechanism 2), its laws are present, verbatim and identical, in every single invocation of that agent, positioned above the task text in the hierarchy the model is trained to respect. A law does not intercept a bad proposal the way the guard hook does; it makes the bad proposal unlikely to be written at all. The fence decides what an agent can touch, and its laws decide what it will refuse to do with what it can touch.

This is why the tutorial calls them _iron laws_ rather than guidelines. Guidelines invite weighing, and a model asked to weigh a guideline against a user's explicit request will often let the request win, politely. A law is written at the top rung of the modal ladder, never, must, only, and it is written to win conflicts: when a task text says just quickly bump the version in build.sbt to the feature-implementer, the collision between that sentence and law 6 (the build is not yours) resolves in the law's favor precisely because the law arrived in the system prompt and the request arrived in the task. The observable result is the boundary probe in [Phase 1](#phase-1-the-factory-builds-the-factory): a refusal that names the owner.

A well-made law has a consistent anatomy, and the agent files in this project follow it deliberately. It states the rule with a binding modal. It names the hazard, the default, or the direction rather than a wish, so it has a failure condition a reviewer can check. It carries its reason inline, in a parenthetical or a trailing clause, because a rule with its reason attached survives paraphrase: whoever rewrites the file can regenerate the correct rule from the reason, and the agent can generalize it correctly to cases the rule did not enumerate. The reference files show the pattern repeatedly: never omit a tools field (omission inherits everything); verify the landed revision (circuit-breaker rollback also reports stable); one val per library family (the version block is the ledger the dependency-updater diffs). And laws are numbered, which looks cosmetic and is not: numbering makes each law citable in reviews, reports, and this tutorial's phase tables, countable at the ratification gate, and stable enough to reference across sessions, which is what lets a failure branch say route the fix per law 2 and mean something precise.

Now the honest limit, which the tutorial never hides: a law is probabilistic. It biases behavior; it does not guarantee it. Laws weaken under long contexts, under artful task framing, under paraphrase drift when files are rewritten, and across model updates. This is exactly why the enforcement ladder exists and why the [authority matrix](#4-the-authority-matrix) has an Enforced-by column: an entry reading instruction only is an honest admission that the limit depends on the model behaving. The design rule that falls out is simple. A law standing alone is acceptable where violation is detectable and reversible, since the reviewer catches it and git reverts it. Anywhere violation is irreversible, the law is paired with code beneath it: the infra-engineer's law against running apply is backed by a deny rule, and the migrator's caution around destructive DDL is backed by the guard hook. The law makes the violation rare; the floor makes the worst violations impossible; the pairing is the design, not redundancy.

Laws also have a place, and misplacing them is the most common authoring error. The channel rule from Phase 0 sorts every candidate sentence by lifetime and audience. A constraint that is timeless and specific to one role is a law in that agent's file. A fact that is timeless and universal, the tier boundaries, upickle only, belongs in CLAUDE.md as a hard rule, where every context loads it. A constraint that applies to one run only, the vacuous-inspect license in [Phase 4](#phase-4-schema-and-data-tier), belongs in task text and nowhere else, so that a one-time exception never hardens into policy. A law that names a current version number is misfiled task text; a hard rule only one agent needs is bloat in everyone else's context.

Finally, where laws come from and how they change. No agent invents its own laws: the factory-engineer transcribes them from the authority matrix, and its first law forbids it from legislating, so a law change is constitutional by definition, matrix first, then file, then your ratification and a restart. But laws do accrete, and the accretion pattern is the system learning. After the doobie import failure, one caution sentence entered the dependency-updater's file; after the circuit-breaker ambiguity, the deploy verification law entered the infra-engineer's. Each genesis failure leaves the factory smarter by exactly one sentence, which is the cheapest form of institutional memory there is. Verification closes the loop: you cannot unit test a sentence, but you can probe the behavior it should produce, and most rows in this tutorial's what-you-observe tables cite a law as their cause. A law that never observably fires is either unnecessary or untested, and the boundary probes exist so that no law in this project is in that state.

The mental model to carry forward, extending the warehouse picture from the tools explanation: tools are the hands and the catalogue, laws are the professional discipline the specialist brings to every job before the job is described, and the floor is the inspector who assumes that discipline sometimes fails. All three are needed because each covers the failure mode of the others.
...
> Use the feature-implementer agent. The test-engineer's report: [PASTE THE FULL N.2 REPORT]. Fix the service tier to satisfy the intended rule it states: remove (Done, Todo) from the legal transition set so Done is terminal. The rules are data; the fix should be a change to the transition set, not new conditional logic. Do not modify any test. Run `sbt check`; report the new transition set verbatim and the full test results.

This is where the Phase 5 insistence on rules as data pays its dividend. Because the transitions live in a `Set` of pairs, the semantic fix is a one-element diff a human can read at a glance, and the verbatim set in the report can be compared against the decided rule without opening a file. If the fix had required restructuring an if-tree, that would itself be evidence that Phase 5's structural instruction had eroded, which the reviewer should hear about.
...
The first fork is the API contract. Requests that used to succeed with 200 now fail with 409. For TaskForge's single frontend that is absorbed in the same change, but the general rule stands: a semantic fix that changes observable API behavior is a contract change, and `TaskRoutesSuite` must visibly update alongside it, which the reviewer treats exactly like a wire-format edit.
...
|---|---|---|
| fix it inline in the main session, it is one line | the orchestrator becomes an unaccountable eleventh owner; no gate sees the change | ownership rule, [section 4](#4-the-authority-matrix) |
| tell the implementer to fix code and tests in one delegation | the same context that misunderstood the rule certifies its own correction; the contradiction is never made visible | writer and checker separation |
| delete the old contradicting test instead of updating it | the wrong rule stops being asserted but the right rule is asserted nowhere; coverage silently narrows | the failing-test oracle |
| catch the symptom in the web tier and return 409 there | the business rule leaks into the presentation tier; the service still believes the wrong rule | tier boundaries in CLAUDE.md |
